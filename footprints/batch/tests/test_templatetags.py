...
class BatchRowTest(TestCase):
    ...
    def test_field_value(self):
        ...
        self.assertEqual(value, 'Eliezer Soncino')

    def test_validate_field_value(self):
        # required field
        fld = BatchRow._meta.get_field('imprint_title')
        row = BatchRowFactory(imprint_title='')
        self.assertEqual(validate_field_value(row, fld, ''),
                         'missing has-error')

        row = BatchRowFactory()
        self.assertEqual(validate_field_value(row, fld, 'Sample'), 'valid')

        # non-required field
        row = BatchRowFactory(footprint_date=None)
        fld = BatchRow._meta.get_field('footprint_date')
        self.assertEqual(validate_field_value(row, fld, None), 'empty')

        row = BatchRowFactory(footprint_date='')
        self.assertEqual(validate_field_value(row, fld, ''), 'empty')

        row = BatchRowFactory(footprint_date='foobar')
        self.assertEqual(validate_field_value(row, fld, 'foobar'),
                         'invalid has-error')

        row = BatchRowFactory(footprint_date=1902)
        self.assertEqual(validate_field_value(row, fld, '1902'), 'valid')
